...
## 最新版本 `3.0` [下载地址](../../releases/download/3.0/qn-ios-ble-sdk-3.0.zip)
* 优化`appId`验证的成功率
* 修复部分设备难以连接的问题
* 增加`lb`、`斤`单位的显示(在连接设备前设置相应的单位，测量结束后SDK返回相对应单位的数值，若设备支持相应单位的显示，则在设备端显示相对应单位和数值)
...
### 使用方法
1. setLogFlag,设置是否打开打印开关。

    ```
    [QingNiuSDK setLogFlag:YES];设置为YES可查看调试信息。
    ```

2. registerApp,在使用Appdelegate中初始化轻牛SDK
...
4. setWeightUnit：设置体重单位

    ```
    //如：设置单位为kg时
    [QingNiuSDK setWeightUnit:QingNiuRegisterWeightUnitKg];如无设备默认为kg。
    ```
...
* 体重单位的类型
    ...
    ```
    typedef NS_ENUM(NSInteger,QingNiuWeightUnit) {//体重单位
